...
class StaffMember(Base):
    ...
    account: Mapped["Account | None"] = relationship(
        back_populates="staff_member", uselist=False
    )
...
class Account(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('active','locked','deactivated')", name="account_status"
        ),
        CheckConstraint(
            "failed_attempts >= 0", name="account_failed_attempts_nonnegative"
        ),
    ...
    staff_member_id: Mapped[UUID] = mapped_column(
        ForeignKey("staff_members.id", ondelete="RESTRICT"), unique=True, nullable=False
    )
    ...
    temporary_pin_expires_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True)
    )
