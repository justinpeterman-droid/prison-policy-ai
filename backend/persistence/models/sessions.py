...
class AccessSession(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "octet_length(access_token_hash) = 32", name="session_access_hash_length"
        ),
        CheckConstraint(
            "octet_length(renewal_token_hash) = 32", name="session_renewal_hash_length"
        ),
        CheckConstraint(
            "octet_length(device_id_hash) = 32", name="session_device_hash_length"
        ),
        CheckConstraint(
            "access_expires_at > created_at", name="session_access_expiry_after_create"
        ),
        CheckConstraint(
    ...
    renewal_expires_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    renewal_family_id: Mapped[UUID] = mapped_column(
...
class RenewalTokenHistory(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "octet_length(token_hash) = 32", name="renewal_history_hash_length"
        ),
        CheckConstraint(
            "expires_at > rotated_at", name="renewal_history_expiry_after_rotation"
        ),
    )
...
class AdminElevation(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "idle_expires_at > issued_at", name="admin_elevation_expiry_after_issue"
        ),
    )
...
class AdminStepUpToken(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "octet_length(token_hash) = 32", name="admin_step_up_hash_length"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "expires_at > issued_at", name="admin_step_up_expiry_after_issue"
        ),
    )
