...
from backend.identity.idempotency import (
    IdempotencyConflict, RequestInProgress, claim_idempotency, complete_idempotency,
    request_digest,
)
...
_CSV_COLUMNS = (
    "event_id", "occurred_at", "actor_account_id", "actor_staff_member_id",
...
def _cursor_key() -> str:
    ...
    if not key:
        raise ApiError("dependency_unavailable", "Pagination is temporarily unavailable.", status=503, retryable=True)
    return key
...
def _statement(filters: dict[str, object]) -> Select:
    ...
    for key, value in filters.items():
        if key == "occurred_at_from": clauses.append(AuditEvent.occurred_at >= value)
        elif key == "occurred_at_to": clauses.append(AuditEvent.occurred_at <= value)
        elif key == "action_family": clauses.append(AuditEvent.action.like(f"{value}.%"))
        else: clauses.append(getattr(AuditEvent, key) == value)
...
def _selection_statement(filters: dict[str, object], reference: dict[str, object] | None = None) -> Select:
    ...
    if reference is not None:
        try:
            ...
            raise ApiError("idempotent_response_unavailable", "Create a new export request.", status=409) from None
...
def list_events():
    ...
    try:
        ...
        raise ApiError("dependency_unavailable", "Audit data is temporarily unavailable.", status=503, retryable=True) from None
...
def export_events():
    ...
    if not isinstance(payload, dict) or set(payload) != {"filters", "format", "reason"}:
        raise ApiError("validation_failed", "Audit export input is invalid.", status=400)
    if payload["format"] != "csv" or not isinstance(payload["reason"], str) or not payload["reason"].strip() or len(payload["reason"]) > 500:
        raise ApiError("validation_failed", "Audit export input is invalid.", status=400)
    ...
    canonical = {"filters": {key: str(value) for key, value in sorted(filters.items())}, "format": "csv", "reason": payload["reason"]}
    try:
        claim = claim_idempotency(
            db, actor, key=key, action="admin.audit_export",
            request_sha256=request_digest(canonical), now=datetime.now(UTC),
        )
        pending = request.headers.get("X-Admin-Step-Up", "")
        try:
            consume_step_up(db, actor=actor, raw_token=pending, purpose="audit_export", now=datetime.now(UTC))
        except (StepUpRequired, ValueError):
            raise ApiError("step_up_required", "Administrator PIN confirmation is required.", status=403) from None
        rows = list(db.scalars(_selection_statement(filters, claim.response_reference if claim.replayed else None).limit(10_001)))
        if len(rows) > 10_000:
            raise ApiError("audit_export_limit_exceeded", "Narrow the audit filters before exporting.", status=409)
        ...
        if not claim.replayed:
            current_app.config["AUDIT_WRITER"].append(
                db,
                AuditEventInput(
                    actor_account_id=actor.account_id, actor_staff_member_id=actor.staff_member_id,
                    action="admin.audit_exported", result="success", request_id=request_id(),
                    target_type="audit_export", target_id=None,
            ...
            if rows:
                ...
                reference = {"export_id": export_id, "selection_end_occurred_at": "1970-01-01T00:00:00Z", "selection_end_event_id": "00000000-0000-0000-0000-000000000000"}
            complete_idempotency(db, claim, response_status=200, response_reference=reference, now=datetime.now(UTC))
        ...
        g.identity_db_failed = True
        raise ApiError("dependency_unavailable", "Audit export is temporarily unavailable.", status=503, retryable=True) from None
