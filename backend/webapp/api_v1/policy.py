...
# One shared pool: a timed-out provider call keeps running (we cannot cancel a
# blocked socket), so it must not pin a per-request executor open.
_provider_pool = ThreadPoolExecutor(max_workers=8, thread_name_prefix="policy")
...
def _call_provider(question: str, history: list[dict[str, str]]) -> dict:
    """Run the existing pipeline under the server budget.

    The call is not cancellable, so a timeout abandons the worker rather than
    pretending to stop it — the point is to bound what the *client* waits for.
    """
    future = _provider_pool.submit(answer_question, question, history=history)
    try:
        return future.result(timeout=POLICY_BUDGET_SECONDS)
    except FutureTimeoutError:
        raise TimeoutError("the policy answer exceeded the server budget") from None
...
def ask_policy_question():
    ...
    _settle(
        ...
        reference={
            ...
            # A digest of the answer proves which answer was delivered without
            # storing a single word of it.
