...
- `wif_trust` is keyed exactly by `terraform-plan`, `terraform-apply`, `deploy`, `rollback`, `admin-bootstrap`, and `access-release`; each value contains `github_environment`, `workflow_refs`, `workflow_claims`, and `ref_pattern` and agrees exactly with the external OP-01 policy. `workflow_claims` is the explicit permitted set of `workflow_ref` and/or `job_workflow_ref`: top-level jobs use `workflow_ref`; reusable jobs use `job_workflow_ref`; only `terraform-plan` may permit both for its manual wrapper and reusable callable job; `terraform-apply` permits only `job_workflow_ref`; deploy, rollback, admin-bootstrap, and access-release permit only `workflow_ref`. Do not unconditionally map an optional reusable-workflow claim into provider attributes.
...
- A `terraform_test_contract` output is permitted solely for credential-free native Terraform assertions. It may expose only static/derived booleans, counts, physical-ID lengths, secret names, role names, workflow-claim categories, and bootstrap environment labels; never a secret value, real identifier, or IAM member. It is not a runtime/client interface and must not be consumed by later tasks.
...
- Apply every OP-03 resource-independent least-privilege boundary: `terraform-plan` is read-only and sees only its OP-02 state prefix; `terraform-apply` receives only its state-prefix write role and reviewed Terraform-management roles; `deploy`, `rollback`, `admin-bootstrap`, and `access-release` remain distinct; no workflow identity reads application secret payloads. The worker alone receives `roles/monitoring.metricWriter` to emit only RP-07's bounded provider-repeat-risk custom metric; no other runtime/workflow account receives that role. Do not invent broad roles before the target resource exists: OP-04 owns the concrete deploy/rollback/release resource bindings, while Vertex/Discovery Engine and managed-signing bindings remain approved external resource-interface gates.
...
- Secret containers are exactly `access-database-url`, `identity-hash-pepper`, `cursor-signing-key`, `client-update-grant-key`, `legacy-access-code`, `legacy-admin-code`, `github-feedback-token`, `flask-session-secret`, and `initial-admin-pin`. Never create a `google_secret_manager_secret_version` instance. A literal block with `for_each = {}` is permitted only to expose a resource-derived zero count to the credential-free native security contract; it must have no instances, secret data, values, provider action, or runtime use.
...
3. Implement services, network, SQL, all ten/eleven identities and six WIF contracts, nine empty secrets, isolated root wiring, mocked native tests, and the human-only runbook. Native Terraform tests must assert actual planned values through the non-secret `terraform_test_contract` output rather than source-text matching: PostgreSQL 17, production HA/deletion protection, 10/11 identity counts and valid physical-ID bounds, 5/6 WIF outputs, exact safe IAM categories, environment-specific bootstrap trust, API-only update-grant access, version-adder-without-accessor, and absence of secret versions.
