...
- Cloud Run IAM cannot distinguish revision-only deployment or traffic-only rollback from another `run.services.update` payload. These protected workflows are the enforcement layer: deploy rejects any configuration, secret, service-account, ingress, scaling, or non-approved-digest change; rollback rejects any configuration/data/secret change and submits only validated prior revision and traffic selections for the named services.
