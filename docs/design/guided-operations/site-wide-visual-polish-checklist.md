...
**Progress note (2026-08-20):** Foundation/Home and shared shell passes were revised against the supplied reference PNG. The implementation adopts its stronger angled shell/hero composition, raised mountain scenery, continuous fence crop, compact action anatomy, content density, and Quick Access rail while excluding the screenshot's invented identity, records, notifications, health details, checklist, activity, quote, date, and version. Runtime candidate `0e1bd6b` passes TypeScript, 181 component tests across 46 files, the production build, and 128 of 129 Chromium officer/admin/responsive/accessibility/print/visual-regression workflows; the one skip accurately reports that this Chromium build cannot emulate `prefers-reduced-data`, while the request-free CSS fallback has a deterministic source-contract test. All 48 Windows visual baselines pass unchanged. Its four-job Foundation gate also passes a built-SPA, Gunicorn, PostgreSQL 17, real-session smoke with fictional data. Local backend evidence includes 41 contract/security tests and 1,440 unit tests (30 skipped). Exact PR head `960944a` passed all 21 checks, repository owner squash merge `8021820` has the identical tree, and a post-merge 1536×1024 real-session browser pass directly rechecked the accepted reference, current Home, and Quick Access-to-Policy-Expert interaction without current-origin console errors. Manual screen-reader, native on-screen-keyboard, physical Windows display-scaling/high-contrast, performance p75, generated-asset production approval, and owner-release acceptance remain open. Checklist reconciliation: 289 of 332 items are complete; 43 remain open.
...
Evidence: `guided-operations.css` owns the shared `--gow-*` canvas, surface, ink, muted, border, focus, disabled, danger, success, warning, and brand tokens; administrator aliases resolve back to those semantic values. The active Sign In action now uses the canonical dark gold-button ink, and a focused contract proves at least 4.5:1 against each of its three actual gradient stops; the previous white-on-gold and too-dark brown-stop combinations are both rejected.
...
Evidence: one shared persistence-status catalog and error classifier now give New Report, Count Sheet, Document Studio copy and officer-report editors, and all six active Daily Paperwork editors the same truthful unsaved, saving, saved, reconnecting, conflict, and failed language. Network-unavailable and revision-conflict errors never claim a save and keep visible input intact; network or terminal failures expose retry controls, while revision conflicts block stale resubmission and direct the employee to copy the visible text before reopening the latest report. Deterministic classifier/component tests cover network, conflict, terminal failure, visible-input preservation, and retry presentation; the browser outage/retry workflow proves `server save not confirmed` followed by `Saved to server` after a successful retry. The shared `StatusMessage` now also supports an unstyled semantic mode: Login authentication failures, Account action/dependency failures, administrator workspace elevation errors, and reusable administrator step-up errors retain their approved copy, element type, and exact feature class names while consistently receiving assertive, atomic live-region behavior. Focused component tests prove those migrations, but warning, confirmation, success, dependency, and announcement patterns are not yet standardized across every active route.
...
Evidence: officer and administrator gold actions now use dark navy text; token tests enforce 4.5:1 contrast for active gold treatments including every Sign In gradient stop, muted-copy, and ten rendered semantic-status pairs, plus a 3:1 focus boundary on common light surfaces. The shared focus token is opaque, forced-colors assertions cover current-page and keyboard-focus semantics, and achromatopsia emulation verifies that administrator status meaning remains explicit in text. Accessibility smoke tests scan every representative officer and administrator route for unnamed interactive controls, missing image alternatives, duplicate IDs, broken labeling/description/error references, positive tabindex, and nested interactive elements. They also measure rendered focus indicators against viewport and overflow-clipping bounds while exercising profile-menu arrow order, mobile-drawer close-to-navigation order and focus restoration, and administrator dialog PIN-to-Cancel order. `officer-route-reflow.spec.ts` covers Forms Library, Policy Expert, and Account controls at 200% and 400% root text size; focused tablet/mobile checks include all Document Studio tabs, the non-shrinking centered 44px Document Studio preview-close control, the Forms selection label, and Random Searches section navigation with measured 44px targets, visible unclipped focus, and logical Tab order. A mobile administrator Accounts test also proves that the Active staff member checkbox retains a 44px label target, a visible 20px native checkbox, and the expected Shift-to-checkbox Tab order. The Monthly Packet browser test measures all four mobile selection labels at 44px or larger, keeps each native checkbox at least 20px, and proves Space-key activation. The mobile Staff Search action, previously measured at 36.328px high, now remains a non-shrinking 44×44 target; its focused browser check proves input-to-button Tab order and Enter-key form submission. Commit `7621a2b` passed all six focused administrator browser workflows, all 62 administrator component tests, and TypeScript. Account now keeps a persistent visually hidden polite, atomic session-list status that announces loading and the exact loaded-session count; it clears on failure so the assertive recovery message remains authoritative. That focused change passed 21 tests across two component files, TypeScript, the Account browser workflow, and the unchanged Account visual baseline. Mobile Roster Drag, Move up, and Move down controls, previously rendered at 27×44, now remain non-shrinking 44×44 targets; the browser test proves each SVG is centered within one CSS pixel and verifies the logical Drag-to-Move-up-to-Move-down Tab sequence. The focused Daily Paperwork browser set passed 3 of 3 and the Roster unit set passed 5 of 5. This durable automated evidence does not claim a complete site-wide target/focus/order audit, native on-screen-keyboard, full WCAG, physical high-contrast, or manual screen-reader acceptance; those remain open.
...
- [x] Require all CI and release gates to pass before merging each visual milestone; exact PR head `960944a` passed all 21 checks, runtime candidate `0e1bd6b` passes its four-job Foundation gate, and repository owner squash merge `8021820` has the identical reviewed tree.

Evidence: runtime candidate `0e1bd6b` passes 181 component tests across 46 files, TypeScript, the Vite production build (26.75 kB CSS gzip and 136.56 kB JavaScript gzip), and 128 of 129 Chromium workflows, including all 48 Windows visual baselines without snapshot updates. Its exact Foundation run also passes browser API contracts, the reviewed artifact, the Cloud Run image build, and two built-SPA real-session workflows against Gunicorn and PostgreSQL 17 with fictional data. The expanded visual set was reviewed directly; it deliberately includes mobile/print references and loaded fictional Reports records. The single skipped workflow is the feature-detected `prefers-reduced-data` browser assertion that current Chromium cannot emulate through CDP; `visual-assets.test.ts` separately proves the request-free reduced-data CSS contract. Local backend evidence includes 41 contract/security tests and 1,440 unit tests with 30 intentional skips. Exact PR head `960944a` passed all 21 checks and repository owner squash merge `8021820` has the identical tree. A post-merge browser pass served that tree through Flask against isolated PostgreSQL 17 fictional accounts, rendered Home at the accepted 1536×1024 size, navigated Quick Access to Policy Expert, and produced no current-origin console warnings or errors. Manual screen-reader, native on-screen-keyboard, physical high-contrast, production p75, and owner-release approval remain separate open gates.
