...
Evidence: one shared persistence-status catalog now gives New Report, Count Sheet, and all six active Daily Paperwork editors the same server-confirmation language. Network-unavailable errors reach `reconnecting` or `offline` without claiming a save, keep the visible draft intact, and preserve the actual error in an alert; deterministic classifier tests and a browser outage/retry workflow prove `server save not confirmed` followed by `Saved to server` after a successful retry.
