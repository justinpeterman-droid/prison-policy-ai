...
- Produces Secret Manager containers named `access-database-url`, `identity-hash-pepper`, `cursor-signing-key`, `client-update-grant-key`, `legacy-access-code`, `legacy-admin-code`, `github-feedback-token`, `flask-session-secret`, and `initial-admin-pin`; no `google_secret_manager_secret_version` instance is permitted. A literal `google_secret_manager_secret_version` block with `for_each = {}` is permitted only to expose a resource-derived zero count to the credential-free native security contract; it has no instances, secret data, values, provider action, or runtime use.
