...
A single-page React app that reproduces six corrections-department documents as
...
| PREA Step Checklist | PREA Step by Step Checklist (2-page) | Checkboxes on the step, paperwork, and E-Omis lists; the email notification list is reference-only |
...
- **Print / Save this form** prints just the active tab; **Print all** prints all six.
- Use your browser's "Save as PDF" print destination to keep a filled copy.

## Editing

- `src.jsx` is the readable JSX source. After editing it, recompile to `app.js`
  with Babel (the committed `app.js` is the pre-compiled output, so no build
  tooling is needed just to *run* the page):
