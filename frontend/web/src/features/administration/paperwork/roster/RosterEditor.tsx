                    ...
                    {zone.posts.map((assignment, index) => {
                      ...
                      return (
                        <tr key={assignment.post_code} onDragOver={(event) => event.preventDefault()} onDrop={() => { const source = dragged.current; if (source?.zoneCode === zone.zone_code) movePost(zone.zone_code, source.postCode, index); dragged.current = null; }}>
                          <td><div className="roster-reorder-controls"><button type="button" draggable aria-label={`Drag ${item.label}`} title={`Drag ${item.label}`} onDragStart={() => { dragged.current = { zoneCode: zone.zone_code, postCode: assignment.post_code }; }}><InterfaceIcon name="drag" /></button><button type="button" aria-label={`Move ${item.label} up`} title={`Move ${item.label} up`} disabled={index === 0} onClick={() => movePost(zone.zone_code, assignment.post_code, index - 1)}><InterfaceIcon name="arrow-up" /></button><button type="button" aria-label={`Move ${item.label} down`} title={`Move ${item.label} down`} disabled={index === zone.posts.length - 1} onClick={() => movePost(zone.zone_code, assignment.post_code, index + 1)}><InterfaceIcon name="arrow-down" /></button></div></td>
