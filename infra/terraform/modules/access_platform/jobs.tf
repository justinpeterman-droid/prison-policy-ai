    ...
    template {
      ...
      containers {
        ...
        env {
          ...
          value = google_secret_manager_secret.initial_admin_pin.id
        }
        volume_mounts {
          name       = "cloudsql"
          mount_path = "/cloudsql"
...
resource "google_service_account_iam_member" "terraform_apply_job_runtime_user" {
  for_each = {
    ...
    bootstrap = google_service_account.bootstrap.name
  }
...
resource "google_cloud_run_v2_job_iam_member" "bootstrap_invoker" {
  ...
  member   = google_service_account.workflow["admin-bootstrap"].member
}
