...
output "terraform_test_contract" {
  value = {
    ...
    observability = {
      ...
      alert_policy_count               = 9 + length(google_monitoring_alert_policy.logical_export_documented) + length(google_monitoring_alert_policy.required)
      logical_export_scheduler_enabled = local.logical_export_scheduler_enabled
      logical_export_schedule          = try(google_cloud_scheduler_job.logical_export_nightly[0].schedule, null)
      ...
      scheduler_workflow_condition     = !local.logical_export_scheduler_enabled || google_project_iam_member.logical_backup_invoker[0].condition[0].expression == "resource.name == \"projects/${var.project_id}/locations/${var.region}/workflows/${google_workflows_workflow.logical_export[0].name}\""
