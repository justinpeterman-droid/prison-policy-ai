...
def test_policy_operation_documents_every_safe_outcome():
    ...
    for status in ("200", "400", "401", "409", "500", "503", "504"):
...
def test_policy_request_schema_is_closed_and_bounded():
    ...
    # The documented bound must match the server's shared cleaner.
    assert history["maxItems"] == 6
...
def test_policy_examples_never_carry_real_looking_identifiers():
    ...
    for leak in ("adc#", "employee_number", "@", "bearer ey", "pin"):
