...
def test_bootstrap_creates_only_one_admin(db_session):
    ...
    staff = StaffMember(employee_number="EMP-9901", rank="Officer", first_name="Avery",
                        last_name="Morgan", shift="A", is_active=True)
    db_session.add(staff)
    db_session.flush()
    audit = RecordingAuditWriter()
    result = bootstrap_first_admin(db_session, staff_member_id=staff.id,
                                   now=datetime(2026, 8, 12, 15, 0, tzinfo=UTC),
                                   audit_writer=audit, operation_id=uuid4(),
    ...
    with pytest.raises(InitialAdminBootstrapRefused):
        bootstrap_first_admin(db_session, staff_member_id=staff.id,
...
def test_account_creation_rolls_back_when_audit_fails(db_session_factory):
    with db_session_factory.begin() as session:
        staff = StaffMember(employee_number="EMP-9902", rank="Officer", first_name="Jordan",
                            last_name="Lee", shift="B", is_active=True)
        session.add(staff)
        session.flush()
        staff_id = staff.id
    try:
        with pytest.raises(RuntimeError, match="audit insert failed"):
            with db_session_factory.begin() as session:
                create_account(session, session.get(StaffMember, staff_id), "user",
                               datetime(2026, 8, 12, 15, 0, tzinfo=UTC),
                               FailingAuditWriter(), "request-create-rollback",
                               uuid4(), uuid4())
        with db_session_factory() as session:
            assert session.scalar(select(func.count(Account.id)).where(
...
def test_bootstrap_advisory_lock_allows_exactly_one_concurrent_success(db_session_factory):
    with db_session_factory.begin() as session:
        session.execute(delete(Account))
        staff_rows = [
            StaffMember(employee_number=f"EMP-991{index}", rank="Officer", first_name="Casey",
                        last_name=f"Tester{index}", shift="A", is_active=True)
            for index in (1, 2)
        ]
        ...
        try:
            with db_session_factory.begin() as session:
                ...
                bootstrap_first_admin(
                    session, staff_member_id=staff_id,
                    now=datetime(2026, 8, 12, 15, 0, tzinfo=UTC),
                    audit_writer=RecordingAuditWriter(), operation_id=uuid4(),
