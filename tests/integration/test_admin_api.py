...
def test_account_create_returns_pin_once_but_database_stores_only_hash(
    ...
    staff = create_staff(
        ...
        audit_writer=audit,
        request_id="request-create-staff-1",
    )
    result = create_account_for_staff(
        ...
        now=identity_fixed_now,
        audit_writer=audit,
        request_id="request-create-account-1",
    )
...
def test_only_active_admin_cannot_demote_self(
    ...
    with pytest.raises(LastActiveAdminError):
        change_account_role_or_status(
            db_session,
            actor=actor,
            target_account_id=fictional_admin_account.id,
            role="user",
            status="active",
            now=identity_fixed_now,
            audit_writer=Audit(),
            request_id="request-last-admin-integration-1",
        )
