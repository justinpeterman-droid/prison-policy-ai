...
from backend.identity.elevation import StepUpRequired, confirm_admin_pin, consume_step_up
...
def test_admin_step_up_is_hashed_session_bound_and_single_use(
    ...
    result = confirm_admin_pin(
        db_session, actor=actor, pin="Q7W9E2", purpose="account_reset_pin",
        now=identity_fixed_now, audit_writer=audit, request_id="request-admin-step-up-1",
    )
    ...
    consume_step_up(
        db_session, actor=actor, raw_token=result.step_up_token,
        purpose="account_reset_pin", now=identity_fixed_now,
    )
