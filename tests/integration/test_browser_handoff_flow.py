...
def test_handoff_replay_is_rejected_and_browser_session_is_attributable(
    ...
    db_session.add(AdminElevation(
        session_id=actor.session_id, issued_at=identity_fixed_now,
    ...
    db_session.add(AdminStepUpToken(
        session_id=actor.session_id, token_hash=hash_token(step_up_token),
        purpose="review_lab_handoff", issued_at=identity_fixed_now,
        expires_at=identity_fixed_now + timedelta(minutes=5),
    ))
    db_session.flush()
    issued = issue_browser_handoff(
        db_session, actor=actor, now=identity_fixed_now,
        audit_writer=Audit(), request_id="request-integration-handoff-1",
        step_up_token=step_up_token,
    )
    redeemed = redeem_browser_handoff(
        db_session, raw_token=issued.token,
        now=identity_fixed_now + timedelta(seconds=1),
        audit_writer=Audit(), request_id="request-integration-handoff-2",
    )
    db_session.flush()
    ...
    with pytest.raises(HandoffInvalid):
        redeem_browser_handoff(
            ...
            audit_writer=Audit(), request_id="request-integration-handoff-3",
        )
