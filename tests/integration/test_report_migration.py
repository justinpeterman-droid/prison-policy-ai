...
REPORT_TABLES = {
    "incidents", "incident_revisions", "reports", "report_access", "report_revisions",
}
...
def test_report_migration_upgrade_downgrade_and_reupgrade(db_engine):
    ...
    # Restore the shared integration database to the repository's full current
    # schema so later modules never inherit this test's deliberate downgrade.
...
def test_report_migration_has_named_constraints_and_query_indexes(db_engine):
    ...
    unique_names = {
        item["name"] for table in ("incident_revisions", "report_revisions")
        for item in inspector.get_unique_constraints(table)
    }
    assert {
        "uq_incident_revisions_parent_number", "uq_report_revisions_parent_number",
    } <= unique_names
    ...
    with db_engine.connect() as connection:
        ...
        """)).scalars().all()
    assert set(triggers) >= {
        "trg_incident_revisions_immutable", "trg_report_revisions_immutable",
    }


def test_duplicate_revision_and_invalid_relationship_are_rejected(
    db_session, fictional_incident, fictional_report, fictional_staff_and_accounts,
):
    first = db_session.query(IncidentRevision).filter_by(
        incident_id=fictional_incident.id, revision_number=1,
    ).one()
    db_session.add(IncidentRevision(
        incident_id=first.incident_id, revision_number=first.revision_number,
        editor_account_id=first.editor_account_id,
        editor_staff_member_id=first.editor_staff_member_id,
        snapshot={"schema_version": 1}, changed_fields={}, reason="autosave",
        client_version="1.0.0", request_id="request_duplicate_revision_1",
    ))
    with pytest.raises(IntegrityError):
        db_session.flush()
    db_session.rollback()

    db_session.add(ReportAccess(
