...
def test_roster_import_is_idempotent_and_preserves_uuid(db_session):
    ...
    db_session.commit()
    original_id = db_session.scalar(select(StaffMember.id).where(StaffMember.employee_number == "EMP-9001"))
    second = import_roster(db_session, FICTIONAL_RECORDS, apply=True)
    db_session.commit()
    current_id = db_session.scalar(select(StaffMember.id).where(StaffMember.employee_number == "EMP-9001"))
