...
def make_incident(session, account: Account, now: datetime | None = None) -> Incident:
    ...
    session.add(
        IncidentRevision(
            ...
            request_id="request_fixture_incident_1",
            created_at=fixed,
        )
    )
    session.flush()
    ...
    *,
    ...
    session.add_all(
        [
            ReportAccess(
                ...
                granted_by_account_id=preparer.id,
                created_at=fixed,
            )
    ...
    session.add(
        ReportRevision(
            ...
            snapshot=content,
            changed_fields={"fields": ["narrative"]},
            reason="manual_save",
            provenance={},
            client_version="1.0.0",
            request_id="request_fixture_report_1",
            created_at=fixed,
        )
    )
    session.flush()
