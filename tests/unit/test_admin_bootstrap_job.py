...
def test_cli_emits_only_the_closed_four_field_result(monkeypatch, capsys):
    ...
    monkeypatch.setattr("backend.jobs.admin_bootstrap.load_bootstrap_request", lambda *args, **kwargs: request)
    monkeypatch.setattr("backend.jobs.admin_bootstrap.create_engine", lambda *args, **kwargs: SimpleNamespace(dispose=lambda: None))
