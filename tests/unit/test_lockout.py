...
def test_reset_and_unlock_clear_lock_state():
    ...
    assert (account.status, account.failed_attempts, account.lock_cycle, account.locked_until) == ("active", 0, 0, None)
...
def test_bootstrap_rejects_non_hash_approval_reference_before_database_use():
    with pytest.raises(ValueError, match="approval reference SHA-256"):
        bootstrap_first_admin(object(), staff_member_id=uuid4(), now=datetime.now(UTC),
                              audit_writer=object(), operation_id=uuid4(),
...
def test_bootstrap_rejects_non_uuid_staff_id_before_database_use():
    with pytest.raises(ValueError, match="staff member ID"):
        bootstrap_first_admin(object(), staff_member_id="not-a-uuid", now=datetime.now(UTC),
                              audit_writer=object(), operation_id=uuid4(),
...
def test_unknown_invalid_employee_still_runs_one_dummy_hash(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr("backend.identity.accounts.verify_pin",
                        lambda encoded, pin: calls.append((encoded, pin)) or False)
    with pytest.raises(InvalidCredentials):
        verify_login_pin(Session(), employee_number="invalid employee", pin="!",
                         now=datetime.now(UTC), audit_writer=Audit(), request_id="request-test")
    assert len(calls) == 1
...
def test_exact_lock_expiry_allows_success_and_resets_cycle(monkeypatch):
    ...
    account = SimpleNamespace(
        id=uuid4(), staff_member_id=staff.id, staff_member=staff, status="locked",
        locked_until=now, failed_attempts=5, lock_cycle=3, must_change_pin=False,
        temporary_pin_expires_at=None, pin_hash="encoded", last_login_at=None,
    )
    ...
    result = verify_login_pin(Session(), employee_number="EMP-9001", pin="Z9Y8",
                              now=now, audit_writer=Audit(), request_id="request-test")
    assert result is account
    assert (account.status, account.failed_attempts, account.lock_cycle,
            account.locked_until, account.last_login_at) == ("active", 0, 0, None, now)
...
def test_current_lock_and_expired_temporary_pin_share_generic_error(monkeypatch):
    ...
    monkeypatch.setattr("backend.identity.accounts.verify_pin", lambda _hash, _pin: True)
    for account in (
        SimpleNamespace(id=uuid4(), staff_member_id=staff.id, staff_member=staff,
                        status="locked", locked_until=now + timedelta(microseconds=1),
                        failed_attempts=5, lock_cycle=1, must_change_pin=False,
                        temporary_pin_expires_at=None, pin_hash="encoded"),
        SimpleNamespace(id=uuid4(), staff_member_id=staff.id, staff_member=staff,
                        status="active", locked_until=None, failed_attempts=0, lock_cycle=0,
                        must_change_pin=True, temporary_pin_expires_at=now, pin_hash="encoded"),
    ):
        with pytest.raises(InvalidCredentials) as caught:
            verify_login_pin(Session(account), employee_number="EMP-9001", pin="Z9Y8",
                             now=now, audit_writer=Audit(), request_id="request-test")
...
def test_fifth_wrong_pin_emits_failure_and_lock_events(monkeypatch):
    ...
    staff = SimpleNamespace(id=uuid4(), employee_number="EMP-9001", is_active=True)
    account = SimpleNamespace(
        id=uuid4(), staff_member_id=staff.id, staff_member=staff, status="active",
        locked_until=None, failed_attempts=4, lock_cycle=0, must_change_pin=False,
        temporary_pin_expires_at=None, pin_hash="encoded",
    )
    ...
    with pytest.raises(InvalidCredentials):
        verify_login_pin(Session(), employee_number="EMP-9001", pin="Z9Y8",
                         now=now, audit_writer=audit, request_id="request-test")
    assert [event.action for event in audit.events] == ["auth.login_failed", "auth.locked"]
    assert audit.events[0].details == {"reason": "invalid_pin"}


def test_login_lock_query_uses_one_inner_join_and_explicit_lock_targets():
    sql = str(_login_account_statement("EMP-9001").compile(
