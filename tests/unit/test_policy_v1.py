...
def _fake_provider(monkeypatch, *, calls=None, result=None, raises=None, delay=0.0):
    ...
    def fake(question, history=None):
        recorded.append({"question": question, "history": history})
        if delay:
            time.sleep(delay)
...
class TestIdempotencyOutcomes:
    ...
    def test_duplicate_while_the_first_call_is_running_is_rejected(
        ...
        def slow(question, history=None):
...
class TestNothingSensitiveIsPersisted:
    ...
    def test_audit_event_records_a_question_digest_not_the_question(
        self, api_client, auth_headers, db_session, monkeypatch,
    ):
        _fake_provider(monkeypatch)
        api_client.post(
        ...
        for _action, details in rows:
            ...
            for leak in (self.SECRET_QUESTION, FICTIONAL_ANSWER, "Fictional passage text"):
                assert leak not in encoded
            assert set(details) <= {"question_sha256", "document_count", "latency_ms"}
            assert details["question_sha256"] == hashlib.sha256(
                self.SECRET_QUESTION.encode("utf-8")
            ).hexdigest()
...
class TestSafeUpstreamErrors:
    ...
    def test_exceeding_the_server_budget_is_a_safe_timeout(
        ...
        _fake_provider(monkeypatch, delay=2.0)
        response = api_client.post(
            "/api/v1/policy/questions",
