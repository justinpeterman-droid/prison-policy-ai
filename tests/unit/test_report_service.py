...
from backend.reports.roster import SqlStaffProvider, StaffProvider, resolve_staff_from_persons
...
def test_public_engine_adapter_signatures_match_the_locked_roadmap():
    ...
    assert [parameter.name for parameter in disciplinary] == ["payload", "staff_provider"]
...
def test_classify_adapter_preserves_the_legacy_response_shape(monkeypatch):
    monkeypatch.setattr(service, "classify_incident", lambda notes: {
...
class _FictionalProvider:
    def lookup(self, name_hint: str):
        if name_hint.strip().lower() in {"example", "alex example"}:
            return {
                "rank": "Officer", "first": "Alex", "last": "Example",
                "employee_number": "TEST-2101", "shift": "A",
            }
        return None
...
def test_extract_adapter_uses_injected_staff_provider_and_keeps_wire_shape(monkeypatch):
    monkeypatch.setattr(service, "extract_slots", lambda notes, category: {
        "date": "08-12-2026", "time": "1400", "location": "Fictional Dayroom",
        "persons": [{"role": "security_staff", "name": "Alex Example", "last": "Example"}],
    })
    monkeypatch.setattr(service, "find_gaps", lambda category, slots: {
        "gaps": [], "markers": [], "checklist": ["fictional_check"],
        "auto_content": ["Fictional approved content."],
    })
    monkeypatch.setattr(service, "compute_provenance", lambda notes, slots: {"date": "notes"})
    monkeypatch.setattr(
        service, "security_staff",
    ...
    assert set(result) == {
        "slots", "gaps", "blocking_remaining", "markers", "checklist",
        "auto_content", "officers", "provenance",
    }
...
def test_disciplinary_adapter_drops_unknown_prior_report_keys(monkeypatch):
    ...
    monkeypatch.setattr(service, "_prepare_generation", lambda payload, staff_provider: context)
    monkeypatch.setattr(
        service, "generate_disciplinary_only",
...
def test_sql_staff_provider_returns_active_bounded_wire_records():
    row = SimpleNamespace(
        id=uuid4(), employee_number="TEST-2101", rank="Officer",
        first_name="Alex", last_name="Example", shift="A", is_active=True,
    ...
    assert provider.search("Example", limit=1) == [{
        "staff_id": str(row.id), "employee_number": "TEST-2101",
        "display_name": "Officer Alex Example", "rank": "Officer",
        "first": "Alex", "last": "Example", "shift": "A", "is_active": True,
    }]
