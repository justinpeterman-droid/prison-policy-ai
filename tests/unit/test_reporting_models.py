...
def test_incident_model_has_current_state_and_search_indexes():
    assert set(Incident.__table__.columns.keys()) >= {
        "id", "created_by_account_id", "created_by_staff_member_id", "status",
        "current_revision_number", "incident_date", "incident_time", "facility",
        "shift", "location", "category", "field_notes", "classification",
        "extracted_facts", "gap_answers", "charges", "validation", "created_at",
        "updated_at", "archived_at",
    }
    for field in ("classification", "extracted_facts", "gap_answers", "charges", "validation"):
    ...
    assert _index_names(Incident) >= {
        "ix_incidents_status_date", "ix_incidents_category_date",
...
def test_report_model_is_one_shared_current_row_with_bounded_type():
    assert set(Report.__table__.columns.keys()) >= {
        "id", "incident_id", "report_type", "reporting_staff_member_id",
        "prepared_by_staff_member_id", "status", "current_revision_number",
        "current_content", "created_by_account_id", "created_at", "updated_at",
        "archived_at",
    }
    ...
    assert _index_names(Report) >= {
        "ix_reports_status_updated", "ix_reports_owner_updated",
        "ix_reports_preparer_updated", "ix_reports_incident",
    }
...
def test_access_and_revision_constraints_are_bounded_and_append_oriented():
    ...
    assert set(ReportRevision.__table__.columns.keys()) >= {
        "source_incident_revision_id", "source_ai_job_id", "fast_model", "pro_model",
        "model_location", "classification_prompt_sha256",
        "generation_prompt_sha256", "checklist_sha256", "template_sha256",
        "cloud_run_revision", "source_commit",
    }
...
def test_all_reporting_foreign_keys_are_explicit():
    expected = {
        ...
        "report_revisions": {
            "reports.id", "accounts.id", "staff_members.id", "incident_revisions.id",
        },
    }
