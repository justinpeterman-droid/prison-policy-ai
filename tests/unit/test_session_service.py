...
from backend.identity.tokens import hash_device_id, hash_token
...
def account_fixture(*, persistent_change=False):
    staff = StaffMember(
        id=uuid4(), employee_number="TEST-1001", rank="Officer",
        first_name="Avery", last_name="Morgan", shift="A", is_active=True,
    )
    account = Account(
        id=uuid4(), staff_member_id=staff.id, role="user", status="active",
        pin_hash="encoded", must_change_pin=persistent_change, failed_attempts=0,
        lock_cycle=0, auth_version=3,
    )
    account.staff_member = staff
    return account


@pytest.mark.parametrize("persistent,renewal_delta", [
...
def test_session_expiries_profile_and_plaintext_boundary(persistent, renewal_delta):
    ...
    pair = create_session(
        session, account, device_id="device-fictional-0001", device_label="  Intake\nDesk  ",
        persistent=persistent, now=NOW, settings=SETTINGS,
    )
...
def test_renewal_rotates_hashes_preserves_nonpersistent_absolute_expiry():
    ...
    issued = create_session(
        setup, account, device_id="device-fictional-0001", device_label="Desk",
        persistent=False, now=NOW, settings=SETTINGS,
    )
    ...
    rotated = renew_session(
        session, supplied_renewal_token=issued.renewal_token,
        device_id="device-fictional-0001", now=NOW + timedelta(hours=1),
        settings=SETTINGS, audit_writer=audit, request_id="request-renew-1",
    )
...
def test_replayed_renewal_revokes_family_and_requires_reauthentication():
    ...
    issued = create_session(
        setup, account, device_id="device-fictional-0001", device_label="Desk",
        persistent=True, now=NOW, settings=SETTINGS,
    )
    current = setup.added[0]
    history = RenewalTokenHistory(
        id=uuid4(), session_id=current.id, renewal_family_id=current.renewal_family_id,
        token_hash=hash_token("historical-token"), rotated_at=NOW,
        expires_at=NOW + timedelta(days=30), reuse_detected_at=None,
    )
    session = FakeSession([None, history, current])
    audit = RecordingAudit()
    with pytest.raises(SessionReauthenticationRequired):
        renew_session(
            session, supplied_renewal_token="historical-token",
            device_id="device-fictional-0001", now=NOW + timedelta(minutes=1),
            settings=SETTINGS, audit_writer=audit, request_id="request-replay-1",
        )
...
def test_list_sessions_returns_only_safe_summary_fields():
    ...
    assert set(summaries[0].__dict__) == {
        "session_id", "device_label", "persistent", "created_at", "last_used_at",
        "renewal_expires_at", "current",
    }
...
def test_resolve_access_session_rejects_auth_version_mismatch():
    ...
    with pytest.raises(SessionReauthenticationRequired):
        resolve_access_session(
            FakeSession([stored, account]), access_token=pair.access_token,
...
def test_revoke_session_is_account_scoped_and_audited():
    ...
    count = revoke_session(
        FakeSession([stored, account], rows=[stored]), actor_account_id=account.id,
        target_session_id=pair.session_id, now=NOW + timedelta(minutes=1),
        audit_writer=audit, request_id="request-revoke-1",
    )
...
def test_revoke_session_refuses_missing_actor_record_before_mutation():
    ...
    with pytest.raises(SessionReauthenticationRequired):
        revoke_session(
            FakeSession([stored, None], rows=[stored]), actor_account_id=account.id,
            target_session_id=stored.id, now=NOW + timedelta(minutes=1),
            audit_writer=RecordingAudit(), request_id="request-revoke-missing-actor",
        )
...
def test_logout_all_increments_version_and_revokes_every_live_session():
    account = account_fixture()
    setup = FakeSession()
    create_session(setup, account, device_id="device-fictional-0001", device_label="Desk",
                   persistent=False, now=NOW, settings=SETTINGS)
    create_session(setup, account, device_id="device-fictional-0002", device_label="Desk 2",
                   persistent=False, now=NOW, settings=SETTINGS)
    rows = setup.added
    audit = RecordingAudit()
    count = logout_all(
        FakeSession([account], rows=rows), account_id=account.id,
        now=NOW + timedelta(minutes=1), audit_writer=audit, request_id="request-logout-all-1",
    )
