...
def test_invented_facts_does_not_flag_a_value_from_the_notes():
    flags = invented_facts(
        ...
        notes="fight at 10:00pm in 8 barracks",
        answers={},
    )
    assert flags == []
...
def test_invented_facts_does_not_flag_values_from_gap_answers():
    flags = invented_facts(
        "Inmate Jones ADC#123456 was charged.",
...
def test_allow_list_suppresses_a_normalized_time():
    ...
    flags = invented_facts(
        "On duty at approximately 10:00pm the fight began.",
        notes="fight ~10pm in 8 barracks",
        answers={},
        allow=["10:00pm"],
    )
    assert flags == []
...
def test_allow_list_suppresses_a_fallback_date():
    ...
    flags = invented_facts(
        "On 08-01-2026 the incident occurred.",
        notes="Inmate Smith fought in the dayroom.",
        answers={},
        allow=["08-01-2026"],
    )
    assert flags == []
...
def test_allow_list_suppresses_a_built_incident_number():
    flags = invented_facts(
        "Incident 2026-08-141 refers.",
        notes="Inmate Smith fought.",
        answers={},
        allow=["2026-08-141"],
    )
    assert flags == []
...
def test_allow_list_ignores_none_and_empty_entries():
    ...
    flags = invented_facts(
        "Inmate Smith ADC#777.",
        notes="Inmate Smith fought.",
        answers={},
        allow=[None, "", "unrelated"],
    )
...
def test_use_of_force_carries_the_409_designation():
    ...
    cat = next(c for c in load_checklist()["categories"] if c["name"] == "use_of_force")
...
def test_no_investigation_signal_means_no_report():
    for slots in ({}, {"investigation_findings": None}, {"investigation_findings": []}):
...
def test_investigation_questions_appear_once_findings_exist():
    ...
    slots = {
        ...
        for g in find_gaps(
            ...
            {
                "persons": _one_inmate(),
                "investigation_findings": ["Reviewed camera footage."],
            },
        )["gaps"]
    }
    assert {
